...
def findCoordinates(hw,overLoads,prayerpots):
    ...
    #Instead of having multiple lists, make a dictionary of type of inventory spot:number of positions and another list that just has all the spots
    #TODO consider making inventory items a class, and potions a subclass and use these objects as dictionary keys
    #TODO consider using key word argument inputs
    coordsList=[]
    coordsDict={}
    time.sleep(5)
    counter=0
    ...
    for i in range(overLoads):
        ...
        counterList.append(counter)
        counter += 1
    coordsDict['overloads']=counterList
    counterList=[]
    for i in range(prayerpots):
        ...
        counterList.append(counter)
        counter += 1
    coordsDict['prayerpots']=counterList
    #TODO try getting rid of counterlist, setting the key to a value of an empty list and then appending to taht
    winsound.Beep(2000,2000)
    print(coordsDict)
    print(coordsList)
    filePath = input("please input a filepath to save the coordinates")
    with open (filePath,'w') as f:
        f.write((str(coordsList)+str(coordsDict)))
    f.close()
